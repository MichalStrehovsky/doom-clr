...
static void load_gradient_gif( void* data, struct GIF_WHDR* whdr ) {
    APP_U32** gradient = (APP_U32**) data;
    if( *gradient != NULL ) return;
    if( whdr->xdim != 256 || whdr->ydim != 1 || whdr->frxd != 256 || whdr->fryd != 1 ) return;
    APP_U32* pixels = (APP_U32*) malloc( 256 * sizeof( APP_U32 ) );
    for( int i = 0; i < 256; ++i ) {
        uint8_t v = whdr->bptr[ i ];
        pixels[ i ] = ( whdr->cpal[ v ].B << 16 ) |( whdr->cpal[ v ].G << 8 ) | ( whdr->cpal[ v ].R );
    }
...
static APP_U32* load_gradient( void ) {
    FILE* fp = fopen( "gradient.gif", "rb" );
    if( !fp ) return NULL;
    fseek( fp, 0, SEEK_END );
    size_t sz = ftell( fp );
    fseek( fp, 0, SEEK_SET );
    ...
    APP_U32* gradient = NULL;
    GIF_Load( data, (long)sz, load_gradient_gif, NULL, (void*)&gradient, 0L );
    return gradient;
}
...
void app_proc( app_t* app, void* user_data )
{
    ...
    APP_U32* gradient = load_gradient();
    if( gradient ) {
